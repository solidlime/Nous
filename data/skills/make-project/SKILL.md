...
| コード・言語マニフェストあり（package.json / pyproject.toml / src/ / *.ts 等） | 明らかにコーディング | 既存 AGENTS.md がなければユーザーに確認してから生成（下記フロー） |
...
#### 確認フロー（常時）
1. AGENTS.md が存在しないことを確認してから「AGENTS.md を生成して `project:<slug>` タグを付けますか？」と提案する
2. 承認は AGENTS.md 生成とタグ付与のみに限る。ファイル一式生成・git 操作（手順 4.7 含む）は手順 1〜4 で改めて確認する。既存ファイル（README.md / .spec/ / CLAUDE.md / GEMINI.md / .gitignore 等）がある場合は上書きせず、追記・スキップ・別名保存をユーザーに確認する（AGENTS.md 保護と同じ原則）
