...
- 既知のトレードオフ: text_delta は推論と同時にストリーム配信されるため、違反時は
  response_replaced 到達までの短時間、修復前の応答がユーザーに見える（ストリーミング UX
  優先の仕様）。判定待ちバッファリングは UX を損なうため行わない。判定対象はキャラ一貫性
  （トーン/順従/性格）であり有害コンテンツの moderation ではない点に注意。
"""
