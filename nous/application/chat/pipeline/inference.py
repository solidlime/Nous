...
class InferenceStep:
    ...
    async def run(
        ...
        messages = await self._build_initial_messages(config, provider, session_messages, turn_ctx)
        ...
        while turn_ctx.tool_call_count <= config.max_tool_calls:
            ...
            current_text = ""
            seg_buf = ["", ""]  # [0]=text accumulator for segment ordering, [1]=thinking accumulator (SPEC R5)
            ...
            self._inject_timestamps(config, messages)

            # Debug: capture the full prompt sent to LLM
            if getattr(config, "debug_mode", False):
                self._dump_debug_prompt(turn_ctx, messages, visible_tools)
            ...
            async for event in provider.stream(
                ...
                if isinstance(event, TextDeltaEvent):
                    ...
                    seg_buf[1] += event.content
                    yield ThinkingDeltaSSE(content=event.content)
                elif isinstance(event, ToolCallEvent):
                    sse = self._handle_tool_call(event, turn_ctx, pending_tool_calls, seen_ids, executed_ids, seg_buf)
                    if sse is not None:
                        yield sse
            ...
            if seg_buf[0]:
                turn_ctx.segments.append({"type": "text", "content": seg_buf[0]})
                seg_buf[0] = ""
            # Flush remaining thinking segment after inner loop (SPEC R5)
            if seg_buf[1]:
                turn_ctx.segments.append({"type": "thinking", "content": seg_buf[1]})
                seg_buf[1] = ""
            ...
            for start_sse in self._build_imagegen_starts(pending_tool_calls):
                yield start_sse

            results = await self._execute_pending_tools(ctx, config, registry, pending_tool_calls, enable_parallel)

            for result_sse in self._handle_tool_results(results, turn_ctx, messages):
                yield result_sse
            ...
            image_parts: list[dict] = self._collect_image_parts(results)

            if image_parts:
                self._append_image_parts_message(provider, messages, image_parts)
            else:
                logger.debug(
    ...
    async def _build_initial_messages(
        ...
        """セッション履歴＋recency digest＋画像/テキストの最新 user メッセージを組み立てる。"""
    ...
    def _inject_timestamps(self, config: ChatConfig, messages: list[LLMMessage]) -> None:
        ...
        """
        if not getattr(config, "show_message_timestamps", False):
            return
    ...
    def _dump_debug_prompt(
        ...
        """デバッグモード時に LLM への投入プロンプト全文を tmp に書き出す。"""
        import tempfile
    ...
    def _handle_tool_call(
        ...
        """ToolCallEvent を処理し、送出すべき ToolCallSSE を返す（スキップ時は None）。

        既に実行済みの tool_use_id（同一ターン内の再送）を排除するため、
        tool_call segment / SSE / pending への記録より先に除外しないと
        「結果無し tool_call」が履歴に永続化され、次ターン以降プロバイダが 400 を返す。
        """
        ...
        return ToolCallSSE(name=event.tool_name, input=event.tool_input, id=event.tool_use_id)

    def _build_imagegen_starts(self, pending_tool_calls: list[ToolCallEvent]) -> list[ImageGenStartSSE]:
        """image_generate ツール実行前にフロントへ送る ImageGenStartSSE を組み立てる。"""
        events: list[ImageGenStartSSE] = []
        for tc in pending_tool_calls:
    ...
    async def _execute_pending_tools(
    ...
    ) -> list[tuple[ToolCallEvent, dict, dict]]:
        """pending tool calls を（並列/直列設定に従って）実行し結果を返す。"""
        ...
        if enable_parallel:
            return list(await asyncio.gather(*[_exec_one(tc) for tc in pending_tool_calls]))
        return [await _exec_one(tc) for tc in pending_tool_calls]

    def _handle_tool_results(
        self,
        results: list[tuple[ToolCallEvent, dict, dict]],
        turn_ctx: ChatTurnContext,
        messages: list[LLMMessage],
    ) -> list[ToolResultSSE | ImageGenResultSSE]:
        """ツール実行結果から SSE・segments・tool_calls_log・LLM 履歴を組み立てる。"""
        events: list[ToolResultSSE | ImageGenResultSSE] = []
        for tc, truncated, tool_result in results:
            ...
            events.append(ToolResultSSE(name=tc.tool_name, result=truncated, id=tc.tool_use_id))
    ...
    def _collect_image_parts(self, results: list[tuple[ToolCallEvent, dict, dict]]) -> list[dict]:
        """ツール実行結果に含まれる画像データを content_parts 用に収集する。"""
    ...
    def _append_image_parts_message(
        self,
        provider: OpenAICompatProvider,
        messages: list[LLMMessage],
        image_parts: list[dict],
    ) -> None:
        """収集した画像データを user メッセージとして履歴へ追加する。

        ビジョン非対応モデルにはテキストのみの代替メッセージを入れる。
        """
        if not provider.supports_vision():
            logger.info(
