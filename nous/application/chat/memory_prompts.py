...
Spec F (2026-09-12) split the single extractor into two prompts:
- _CONTEXT_LLM_PROMPT: facts / goals / promises / context_update (incl. emotion/body/state)
- _ITEM_LLM_PROMPT: inventory_update only
_MEMORY_LLM_PROMPT is kept as a backward-compatible alias of the context prompt.
"""
...
以下の会話から、記憶・状態の更新情報を抽出してください。感情・身体・状態の更新はこの抽出の担域です。
...
- 何も抽出すべきものがなければ {{"facts": [], "goals": [], "promises": [], "context_update": {{}}}} を出力。
"""
...
以下の会話から、所持品（アイテム）の更新情報を抽出してください。
...
- アイテムの変化がなければ {{"inventory_update": {{}}}} を出力。
"""

# 後方互換エイリアス (旧名は context 側プロンプトを指す)
_MEMORY_LLM_PROMPT = _CONTEXT_LLM_PROMPT
