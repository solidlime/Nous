...
def render_persona_js() -> str:
    ...
    return (
        ...
        "    if (!grid) return;\n"
        "    safeSetHTML(grid, '<div style=\"text-align:center;color:var(--text-muted);padding:40px\">Loading personas...</div>');\n"
        "    try {\n"
        "        var listData = await api('/api/personas');\n"
        "        var personas = listData.personas || [];\n"
        "        if (personas.length === 0) {\n"
        "            safeSetHTML(grid, '<div style=\"text-align:center;color:var(--text-muted);padding:40px\">No personas found. Create one!</div>');\n"
        "            return;\n"
        "        }\n"
        ...
        "    } catch (e) {\n"
        "        safeSetHTML(grid, '<div style=\"text-align:center;color:#ef4444;padding:40px\">Failed to load personas: ' + esc(e.message) + '</div>');\n"
        "    }\n"
        "}\n"
        "\n"
        "// --- Create Persona ---\n"
