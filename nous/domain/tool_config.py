...
class ToolConfig(BaseModel):
    ...
    image_gen_comfyui_timeout_seconds: int = Field(
        default=180, description="画像生成のタイムアウト（秒）。"
    )  # 生成タイムアウト（秒）
    ...
    image_gen_comfyui_workflow_name: str = Field(
        default="", description="ComfyUI側のワークフロー名。"
    )  # workflow_source="comfyui" 時の ComfyUI 側ワークフローファイル名
    ...
    image_gen_negative_prompt: str = Field(
        default="", description="生成から除外したい内容（ネガティブプロンプト）。"
    )  # negative prompt for image generation
    image_gen_full_body_prefix: str = Field(
        default="full body, standing, pov, ", description="全身構図のプロンプト接頭辞。"
    )
    image_gen_portrait_prefix: str = Field(
        default="upper body, portrait, pov, ", description="上半身構図のプロンプト接頭辞。"
    )
    image_gen_selfie_prefix: str = Field(
        default="selfie, from below, mirror selfie, ", description="自撮り構図のプロンプト接頭辞。"
    )
    image_gen_scene_prefix: str = Field(
        default="environment shot, full body, ", description="情景構図のプロンプト接頭辞。"
    )

    # Image caption (for non-vision providers)
    image_caption_enabled: bool = Field(
        default=True, description="画像をテキストで説明するキャプション機能を有効にします。"
    )
    ...
    emotion_decay_half_life_hours: float | None = Field(
        default=None, description="感情価が半分に減衰するまでの時間。空欄でカテゴリ既定。"
    )
