...
- **Root cause #1**: `chat_stream.py:_do_chat` で `ctx.search_engine` が Qdrant 初期化失敗時に None になるのに、`set_persona()` を無チェックで呼び → AttributeError → SSE 空切断
- **Root cause #2**: `service.chat()` に catch-all try/except がなく、パイプライン中盤の例外が ErrorSSE に変換されず接続切断
- **Fix**: (1) search_engine None ガード追加、(2) chat() 全体 try/except + ErrorSSE yield、(3) prepare.py の asyncio.gather に return_exceptions=True 追加

### 教訓
1. **SSEストリーミングの catch-all 必須**: 最初の yield 以降の例外は自動で ErrorSSE にならない。全体を try/except で包むこと。
2. **asyncio.gather の return_exceptions**: デフォルト False は1つのタスク失敗で全結果喪失。安全側に倒せ。
3. **外部依存の None 伝播**: Qdrant 失敗で search_engine が None になるパターンでは、全呼び出し元でチェック必須。
