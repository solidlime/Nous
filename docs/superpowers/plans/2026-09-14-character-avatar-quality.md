...
- モデルは正常に描画される（`_herta_shot_before.png` でテクスチャ・MToon とも正常）。リポジトリ直下 `_avatar_dev_1.png`(3.3KB 全白, 20:34:17) は dev harness 構築途中の産物で**製品の不具合ではない**。
