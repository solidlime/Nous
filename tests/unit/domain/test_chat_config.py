...
class TestAvatarConfigFileRoundTrip:
    """Avatar 設定ブロック（R6）: config.json 往復・後方互換・フラット化."""

    def _write_config(self, tmp_path, persona, data: dict) -> None:
        import json

        path = tmp_path / "persona" / persona
        path.mkdir(parents=True, exist_ok=True)
        (path / "config.json").write_text(json.dumps(data), encoding="utf-8")

    def test_backward_compat_no_avatar_block(self, tmp_path):
        """avatar ブロック無しの既存 config.json はエラーにならず既定値."""
        from nous.domain.chat_config import ChatConfigFileRepository

        self._write_config(tmp_path, "legacy", {"persona": "legacy", "language": "ja"})
        cfg = ChatConfigFileRepository(str(tmp_path)).get("legacy")
        assert cfg.avatar.enabled is False
        assert cfg.avatar.panel_width == 220
        assert cfg.language == "ja"  # 他設定は無傷

    def test_round_trip_avatar_block(self, tmp_path):
        """avatar ブロック付き config.json → save → 再読込で維持される."""
        import json
        ...
        assert cfg.avatar.panel_position == "bottom"
        assert cfg.avatar.mouth_mode == "toggle"
        assert cfg.avatar.panel_width == 320

        # save 後も avatar ブロックが維持される
        repo.save(cfg)
        saved = json.loads((tmp_path / "persona" / "p" / "config.json").read_text(encoding="utf-8"))
        assert saved["avatar"] == {
            "enabled": True,
            "panel_position": "bottom",
            "mouth_mode": "toggle",
    ...
    def test_chat_config_flattened_dump_contains_avatar(self):
        """ChatConfig.model_dump() のフラット化で avatar ブロックがトップレベルに出る."""
        from nous.domain.chat_config import ChatConfig

        cfg = ChatConfig(persona="p", avatar={"enabled": True, "panel_width": 300})
        d = cfg.model_dump()
        assert d["avatar"] == {"enabled": True, "panel_position": "top", "mouth_mode": "analyser", "panel_width": 300}
