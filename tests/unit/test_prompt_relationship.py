...
from nous.application.chat.pipeline.prompt import PromptBuildStep, _build_relationship_context
...
def _patch_datetime():
    with patch("nous.application.chat.pipeline.prompt.datetime", FakeDatetime):
        yield
...
class TestBuildRelationshipContext:
    ...
    def test_no_history_returns_empty(self):
        ...
        result = _build_relationship_context(db, "test_persona")
        assert result == ""
    ...
    def test_context_appended_to_prompt(self):
        """Verify _build_relationship_context output appears in final system_prompt."""
        first_at = _sqlite_dt(REF_NOW - timedelta(days=3))
        mock_db = MockDb(first_at=first_at, active_days=2)
        mock_conn = MagicMock()
        mock_conn.get_memory_db.return_value = mock_db
        ...
        assert "関係性コンテキスト" in turn_ctx.system_prompt
