...
# ---------------------------------------------------------------------------
# chat_management.py — avatar asset serving (PNGTuber R5)
# ---------------------------------------------------------------------------


@pytest.mark.integration
class TestAvatarAssetServe:
    """GET /api/chat/{persona}/persona/avatar/{filename}."""

    async def test_serve_avatar_png(self, client, tmp_data_dir):
        import json
        from pathlib import Path

        avatar_dir = Path(tmp_data_dir) / "persona" / "avatar_persona" / "avatar"
        avatar_dir.mkdir(parents=True, exist_ok=True)
        # PNG ヘッダ相当の最小バイト列（配信されるかだけを見る）
        (avatar_dir / "base.png").write_bytes(b"\x89PNG\r\n\x1a\n" + b"0" * 16)

        resp = await client.get("/api/chat/avatar_persona/persona/avatar/base.png")
        assert resp.status_code == 200
        assert resp.content.startswith(b"\x89PNG")
        assert resp.headers.get("content-type") == "image/png"

    async def test_missing_file_returns_404(self, client, tmp_data_dir):
        resp = await client.get("/api/chat/avatar_missing/persona/avatar/nonexistent.png")
        assert resp.status_code == 404

    async def test_non_png_rejected(self, client, tmp_data_dir):
        from pathlib import Path

        avatar_dir = Path(tmp_data_dir) / "persona" / "avatar_bad" / "avatar"
        avatar_dir.mkdir(parents=True, exist_ok=True)
        (avatar_dir / "evil.txt").write_text("not an image")
        resp = await client.get("/api/chat/avatar_bad/persona/avatar/evil.txt")
        assert resp.status_code == 400

    async def test_path_traversal_rejected(self, client, tmp_data_dir):
        resp = await client.get("/api/chat/avatar_trav/persona/avatar/..%2Fconfig.json")
        assert resp.status_code in (400, 404)
        resp = await client.get("/api/chat/avatar_trav/persona/avatar/..%2Fconfig.json.png")
        assert resp.status_code in (400, 404)

    async def test_avatar_block_in_chat_config(self, client, tmp_data_dir):
        """GET /api/chat/{persona}/config に avatar 設定ブロックが含まれる（R6）。"""
        persona = "avatar_cfg"
        resp = await client.get(f"/api/chat/{persona}/config")
        assert resp.status_code == 200
        data = resp.json()
        assert data["avatar"] == {
            "enabled": False,
